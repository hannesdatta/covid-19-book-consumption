...
# filter for only users with their Gender defined
user_info_to_add<-user_info%>%select(user_id, Male)
all_books<- all_books %>% left_join(user_info_to_add, by=c("reader id"="user_id"))
all_books<- all_books %>% filter(!is.na(Male))
user_info<-user_info%>%filter(!is.na(Male))

################################################################################
################################################################################
################################################################################
# start with examining the number of books added per user per week, only looking at the people who have added their gender.
...
write.csv2(as.data.frame(summary(regression_nr_books)$coefficients), file = "../../gen/output/model1_gender_qty.csv", fileEncoding = "UTF-8")

#next, with the gender interaction:
regression_nr_books <- lm((total)~ value+week+date+Country+Male+Male*value, weekly_per_user_complete)
summary(regression_nr_books)
write.csv2(as.data.frame(summary(regression_nr_books)$coefficients), file = "../../gen/output/model2_gender_qty.csv", fileEncoding = "UTF-8")
...
data_to_add<-user_info%>% select(user_id,Country, Male)
...
write.csv2(as.data.frame(summary(regression_days)$coefficients), file = "../../gen/output/model1_gender_days.csv", fileEncoding = "UTF-8")


#now with the interaction:
regression_days <- lm((days)~ value+week+first_day_of_week_added+Country+Male+Male*value, days_per_user)
summary(regression_days)
write.csv2(as.data.frame(summary(regression_days)$coefficients), file = "../../gen/output/model2_gender_days.csv", fileEncoding = "UTF-8")
...
write.csv2(as.data.frame(summary(regression_rating)$coefficients), file = "../../gen/output/model1_gender_rating.csv", fileEncoding = "UTF-8")
...
write.csv2(as.data.frame(summary(regression_length)$coefficients), file = "../../gen/output/model1_gender_length.csv", fileEncoding = "UTF-8")

#with interaction:
regression_length <- lm((length)~ value+week+first_day_of_week_added+Country+Male*value+Male, length_per_user)
summary(regression_length)
write.csv2(as.data.frame(summary(regression_length)$coefficients), file = "../../gen/output/model2_gender_length.csv", fileEncoding = "UTF-8")
...
write.csv2(as.data.frame(summary(regression_nostalgic)$coefficients), file = "../../gen/output/model1_gender_nostalgic.csv", fileEncoding = "UTF-8")
...
write.csv2(as.data.frame(summary(regression_recent)$coefficients), file = "../../gen/output/model1_gender_recent.csv", fileEncoding = "UTF-8")
